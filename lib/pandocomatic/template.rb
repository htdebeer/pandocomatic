...
module Pandocomatic
    ...
    class Template 
        ...
        SECTIONS = [EXTENDS, GLOB, SETUP, PREPROCESSORS, METADATA, PANDOC, POSTPROCESSORS, CLEANUP]
        ...
        #
        # @return [Bool]
        def has_preprocessors?()
            has_section?(PREPROCESSORS)
        end
        ...
        #
        # @return [Bool]
        def has_postprocessors?()
            has_section?(POSTPROCESSORS)
        end
        ...
        def has_section?(name)
            @data[name] and not @data[name].empty?
        end
